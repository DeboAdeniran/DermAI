...
const PHOTO_GUIDELINES = [
  ...
  "High resolution (at least 800×800px)",
];
...
function AnalysisStep({ label, isActive, isComplete }) {
  return (
    <div className="flex items-center gap-3">
      <div
        className={`w-7 h-7 rounded-full flex items-center justify-center transition-all shadow-sm ${isComplete ? "bg-green-500" : isActive ? "bg-gradient-to-br from-[#8b7355] to-[#6d5a43]" : "bg-[#e8e6e3]"}`}
      >
        {isComplete ? (
      ...
      <span
        className={`text-sm font-light ${isComplete || isActive ? "text-[#2a2420]" : "text-[#5a5450]"}`}
      >
        {label}
      </span>
    </div>
...
export function AnalysisPage() {
  ...
  const capturePhoto = () => {
    ...
    setImageQuality(
      canvas.width >= 800 && canvas.height >= 600 ? "good" : "poor",
    );
    // Convert to File
    canvas.toBlob(
      (blob) => {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () =>
        setImageQuality(
          img.naturalWidth >= 800 && img.naturalHeight >= 600 ? "good" : "poor",
        );
  ...
  // ── ANALYZING SCREEN ────────────────────────────────────────────────────────
  if (step === "analyzing") {
    const activeStepIdx = ANALYSIS_STEPS_UI.findIndex(
      (s) => analysisProgress < s.completeAt,
    );
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#e8e6e3] via-[#f0ede8] to-[#f5f3ef] flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 sm:p-12 max-w-md w-full border border-[#e8e6e3]/30">
          <div className="text-center mb-8">
            <div className="w-24 h-24 mx-auto mb-6 relative">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 96 96">
                ...
                <circle
                  ...
                  strokeWidth="8"
                  fill="none"
                  strokeDasharray={`${(analysisProgress / 100) * 264} 264`}
                  strokeLinecap="round"
            ...
            <h2 className="text-2xl text-[#2a2420] mb-2 font-light">
              Analysing Your Skin
            </h2>
            <p className="text-sm text-[#5a5450] font-light">
              Our AI is reviewing your photo…
            </p>
          </div>
          <div className="space-y-3">
            {ANALYSIS_STEPS_UI.map((s, i) => (
              <AnalysisStep
                ...
                isActive={
                  i === activeStepIdx && analysisProgress < s.completeAt
                }
              />
            ))}
          </div>
          <p className="text-xs text-[#8b7355] text-center mt-6 font-light">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#e8e6e3] via-[#f0ede8] to-[#f5f3ef] flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-xl text-[#2a2420] font-light mb-2">
            Analysis Failed
          </h2>
          <p className="text-sm text-[#5a5450] font-light mb-6">
            {errorMessage}
          </p>
          <button
            onClick={() => {
          ...
          <div className="bg-white rounded-3xl shadow-xl p-6 sm:p-8 border border-[#e8e6e3]/30">
            <div className="flex items-center gap-3 mb-6">
              <img
                src={capturedImage}
                alt="preview"
                className="w-16 h-16 rounded-2xl object-cover border-2 border-[#8b7355]/20"
              />
              <div>
                ...
                <p className="text-sm text-[#5a5450] font-light">
                  Helps our AI give better results
                </p>
              </div>
            </div>
            ...
            {/* Concerns */}
            <div className="mb-6">
              <label className="block text-sm text-[#2a2420] font-light mb-3">
            ...
            {/* Sun exposure */}
            <div className="mb-6">
              <label className="block text-sm text-[#2a2420] font-light mb-3 flex items-center gap-2">
                <Sun size={14} className="text-[#8b7355]" /> Sun exposure level
              </label>
              <div className="flex flex-wrap gap-2">
                {SUN_EXPOSURE.map((s) => (
                  <button
                    key={s}
                    onClick={() => setSunExposure(sunExposure === s ? "" : s)}
            ...
            <div className="mb-6">
              <label className="block text-sm text-[#2a2420] font-light mb-3 flex items-center gap-2">
                <Droplets size={14} className="text-[#8b7355]" /> Current
                climate / humidity
              </label>
              <div className="flex flex-wrap gap-2">
                {CLIMATE_TYPES.map((c) => (
                  <button
                    key={c}
                    onClick={() => setHumidity(humidity === c ? "" : c)}
            ...
            {/* Recent changes */}
            <div className="mb-4">
              <label className="block text-sm text-[#2a2420] font-light mb-2">
                Recent skin changes (optional)
              </label>
              <textarea
                value={recentChanges}
                onChange={(e) => setRecentChanges(e.target.value)}
                rows={2}
                placeholder="e.g. breakout after changing products last week…"
            ...
            <div className="mb-6">
              <label className="block text-sm text-[#2a2420] font-light mb-2">
                Product reactions / allergies (optional)
              </label>
              <textarea
                value={productReactions}
                onChange={(e) => setProductReactions(e.target.value)}
                rows={2}
                placeholder="e.g. reacts to fragrance, sensitive to retinol…"
                className="w-full px-4 py-3 bg-[#f8f6f3] rounded-xl border border-[#e8e6e3] text-sm font-light focus:outline-none focus:ring-2 focus:ring-[#8b7355]/30 resize-none"
              />
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 border border-[#e8e6e3] text-[#5a5450] rounded-xl text-sm font-light hover:bg-[#f8f6f3] transition-all"
              >
                Back
              </button>
              <button
                onClick={submitAnalysis}
                className="flex-1 py-3 bg-gradient-to-r from-[#8b7355] to-[#6d5a43] text-white rounded-xl text-sm font-light flex items-center justify-center gap-2 hover:shadow-lg transition-all"
              >
                Analyse My Skin
              </button>
            </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#e8e6e3] via-[#f0ede8] to-[#f5f3ef]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 pt-28">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl text-[#2a2420] font-light mb-2">
            AI Skin Analysis
          </h1>
          <p className="text-[#5a5450] font-light text-sm">
            Upload a clear photo of your face for a personalised skin report
          </p>
        </div>

        {/* Photo guidelines */}
        <div className="bg-white rounded-2xl p-5 mb-5 border border-[#e8e6e3]/30 shadow-sm">
          <h3 className="text-sm text-[#2a2420] font-light mb-3 flex items-center gap-2">
            <Zap size={14} className="text-[#8b7355]" /> For best results
          </h3>
          <div className="grid grid-cols-2 gap-2">
            {PHOTO_GUIDELINES.map((g, i) => (
              ...
              >
                <CheckCircle
                  size={12}
                  className="text-[#8b7355] mt-0.5 shrink-0"
                />{" "}
                {g}
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[#e8e6e3]/30 overflow-hidden mb-5">
          {capturedImage ? (
            <div className="relative">
              ...
              {imageQuality === "poor" && (
                <div className="absolute bottom-3 left-3 right-3 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 flex items-center gap-2 text-xs text-amber-700 font-light">
                  <AlertTriangle size={12} /> Low resolution — results may be
                  less accurate
                </div>
              )}
              {imageQuality === "good" && (
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-3">
                <button
                  ...
                  className="w-14 h-14 bg-white rounded-full shadow-xl flex items-center justify-center border-4 border-[#8b7355]"
                >
                  <Camera size={22} className="text-[#8b7355]" />
                </button>
                <button
                  onClick={stopCamera}
                  className="w-10 h-10 bg-white/80 rounded-full shadow flex items-center justify-center"
                >
                  <X size={16} className="text-[#2a2420]" />
                </button>
              </div>
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-[#5a5450] font-light text-sm mb-6">
                No photo selected
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  onClick={startCamera}
                  className="flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-[#8b7355] to-[#6d5a43] text-white rounded-full text-sm font-light hover:shadow-lg transition-all"
                >
                  <Camera size={16} /> Use Camera
                </button>
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="flex items-center justify-center gap-2 px-5 py-3 border border-[#e8e6e3] text-[#5a5450] rounded-full text-sm font-light hover:bg-[#f8f6f3] transition-all"
                >
